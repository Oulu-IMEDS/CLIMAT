...
def proc_targets(df, *args, **kwargs):
    ...
    df = df.replace('prog_inter_1', 1.0)
    df[f'DT'] = list(dt)

    for grading in grading_types:
        for i in range(lower_y, upper_y):
            prog_key = f'Progressor_{grading}_{i}y'
            grade_key = f'{grading}_{i}y'

            if prog_key not in df.columns:
                df[prog_key] = None
            if grade_key not in df.columns:
                df[grade_key] = None

            df[prog_key] = df[prog_key].astype(float)
        ...
        for i in range(lower_y, upper_y):
            prog_key = f'Progressor_{grading}_{i}y'
            grade_key = f'{grading}_{i}y'
            progression.append(df[prog_key].tolist())
        ...
        progression = np.transpose(progression, (1, 0))
        # Denote missing values by -1
        progression = np.nan_to_num(progression, nan=0.5)
        ...
        # 1: not missing
        # 0: missing
        prog_mask = copy.deepcopy(progression)
        prog_mask[prog_mask != 0.5] = 1
        prog_mask[prog_mask == 0.5] = 0

        prognosis_mask = copy.deepcopy(prognosis)
        prognosis_mask[prognosis_mask != -1] = 1
        prognosis_mask[prognosis_mask == -1] = 0

        df[f'progs_{grading}'] = list(progression)
        df[f'progs_mask_{grading}'] = list(prog_mask)
        df[f'prognosis_{grading}'] = list(prognosis)
        df[f'prognosis_mask_{grading}'] = list(prognosis_mask)
        df.fillna(inplace=True, value={f'first_prog_{grading}': upper_y + 1})
...
def load_metadata(cfg, proc_targets=None, eval_only=False):
    img_root = cfg.root
    ...
    if os.path.isfile(pkl_meta_fullname):
        if eval_only:
            log.info(f'Read meta file {pkl_meta_oai_site_test_fullname}')
            with open(pkl_meta_oai_site_test_fullname, 'rb') as f:
                oai_meta_test = pickle.load(f)
                return oai_meta_test
        else:
            ...
            with open(pkl_meta_fullname, 'rb') as f:
                ...
                oai_meta_test = loaded_data['oai_site_test']
                most_meta = loaded_data['most_test']
    else:
        log.info(f'Cannot find meta file {pkl_meta_fullname}. Creating new file...')

        oai_meta_all = pd.read_csv(oai_filename)

        print(f'Loaded OAI entries: {len(oai_meta_all.index)}')
        oai_meta_all = filter_df_by_targets(oai_meta_all, cfg)
        print(f'After filterred OAI entries: {len(oai_meta_all.index)}')
        ...
        if proc_targets is not None:
            ...
            print(f'Original OAI test data: {len(oai_meta_test.index)}')
            oai_meta_test = proc_targets(oai_meta_test)
        ...
        if proc_targets is not None:
            ...
            oai_meta = proc_targets(oai_meta)
        ...
        loaded_data = {'oai_site_folds': split_data, 'oai_site_train': oai_meta, 'oai_site_test': oai_meta_test}
    ...
    return split_data, oai_meta, oai_meta_test, most_meta
...
def parse_item_progs(root, entry, trf, **kwargs):
    ...
    prognosis = np.expand_dims(entry[kwargs['prognosis']], axis=1).astype(np.float32)
    prognosis_mask = entry[kwargs['prognosis_mask']]
    ...
    if "DT" in kwargs["metadata"]:
        input['DT'] = np.array([entry[kwargs['DT']]]).astype(np.float32)

    has_grading = entry[grading] is not None and entry[grading] == entry[grading]

    n_gradings = MAX_GRADES[grading] + 1

    if grading in kwargs["metadata"] and has_grading:
        gradings = [0.0] * n_gradings
        grading_value = int(entry[grading])
        gradings[grading_value] = 1.0
        grading_prog_mask = [i >= grading_value for i in range(n_gradings)]
        input[grading] = torch.tensor(gradings)
        input[f"{grading}_mask"] = torch.tensor(grading_prog_mask)

    meta = parse_metadata(entry, kwargs["metadata"])
    input.update(meta)

    return {'data': {'input': input},
            grading: torch.tensor(int(entry[grading])) if has_grading else torch.tensor(-1),
            f'{grading}_mask': has_grading,
            'prognosis': prognosis,
            'prognosis_mask': torch.from_numpy(prognosis_mask.copy()) > 0}


def store_model(epoch_i, task_name, metric_name, metrics, stored_models, model, saved_dir, cond="max", mode="avg"):
    if isinstance(metric_name, str):
        if "." in metric_name:
            _tmp = []
            for name in metric_name.split("."):
                _tmp.append(np.array(metrics[task_name][name]))
            _tmp = np.stack(_tmp, axis=0)
            if len(_tmp.shape) == 1:
                metric_values = np.mean(_tmp)
            elif len(_tmp.shape) == 2:
                metric_values = np.mean(_tmp, axis=1)
            else:
                raise ValueError(f'Not support `metric_values` with {len(_tmp.shape)}-D.')
        else:
            metric_values = metrics[task_name][metric_name]
        if mode == "avg":
            cur_metric = np.nanmean(np.array([v for v in metric_values if v is not None]))
        elif "beta" in mode:
            cur_metric = metric_values
            num_metric = len(cur_metric)
            cur_metric = np.array([0.0 if m is None else m for m in cur_metric])
            xticks = np.array([i + 1 for i in range(num_metric)]) / num_metric
            if "rev" in mode:
                xticks = xticks[::-1]
            a = 4
            b = 1
            w = beta.pdf(xticks, a, b)
            w = w / w.sum()
            cur_metric = np.sum(w * cur_metric)
        elif mode == "linear":
            cur_metric = metric_values
            num_metric = len(cur_metric)
            cur_metric = np.array([0.0 if m is None else m for m in cur_metric])
            w = np.array([i + 1 for i in range(num_metric)])
            w = w / w.sum()
            cur_metric = np.sum(w * cur_metric)
        elif mode == "scalar":
            cur_metric = metric_values
        ...
        if check_cond(cur_metric, stored_models[task_name][metric_name]['best'], cond):
            stored_models[task_name][metric_name]['best'] = cur_metric

            # Remove prev model
            prev_model_fullname = os.path.join(saved_dir, stored_models[task_name][metric_name]['filename'])
            ...
            # Save improved model
            stored_models[task_name][metric_name]['filename'] = \
                "model_" + f"{epoch_i:03d}" + "_" + task_name + "_" + mode + "_" + metric_name + "_" + f'{cur_metric:.03f}' + ".pth"
            saved_model_fullname = os.path.join(saved_dir, stored_models[task_name][metric_name]['filename'])
            saved_log_fullname = saved_model_fullname[:-4] + ".json"
            torch.save(model.state_dict(), saved_model_fullname)
            if "." in metric_name:
                results = {mode: cur_metric}
                for name in metric_name.split("."):
                    results[name] = metrics[task_name][name]
            else:
                results = {metric_name: metrics[task_name][metric_name], mode: cur_metric}
            with open(saved_log_fullname, "w") as f:
                json.dump(results, f)
    else:
        print(f'Skip checking for storing model for empty metric {metric_name}.')
...
def parse_metadata(entry, metadata):
    ...
    if "AGE" in metadata:
        ...
        for i in range(len(age_ranges) - 1):
            if age_ranges[i] <= age_ori < age_ranges[i + 1]:
                ...
                break
        if age_level is None:
            raise ValueError(f'Cannot find AGE level of {age_ori}.')
        age = [0.0] * n_segments
    ...
    if "BMI" in metadata:
        ...
        if bmi_level is None:
            raise ValueError(f'Cannot find BMI level of {bmi_ori}.')
        bmi = [0.0] * 4
        bmi[bmi_level] = 1.0
